...
- `sync.sh` — the everyday command. Composes `config/pull.sh` → `profile/sync.sh` → `brew/sync.sh` → `system/sync.sh`, in that order. `RNFMAC_SYNC_PROFILES_ONLY=1` skips the brew/system steps.
...
- `upgrade.sh` — downloads the latest GitHub release tarball, installs it as a new version dir, flips `current`, and pulls configuration without applying sync.
...
- `config/` — `pull.sh` fast-forwards the clean persistent `~/.rn-forge/macsetup/config` checkout from `origin/main`; `status.sh` reports its revision and changes; `push.sh` requires `-m`, commits all config changes, and pushes directly to the linear `main` branch only when local HEAD equals `origin/main`.
...
**Machine configuration** lives in the separate `macsetup-config` repository, cloned to `~/.rn-forge/macsetup/config`: `shared/profile.zsh` + `shared/aliases.zsh`, and `hosts/<hostname>/profile.zsh` + `Brewfile`. At sync time, shared + host `profile.zsh` are concatenated (host wins by coming last). Stable theme and macOS keybinding assets remain bundled under `src/profiles/shared/`.
